...
for ((k=1;k<=$RBD_NUMBER_PER_MACHINE;k++));
do
 rbd map $POOL_NAME/"$MACHINE_NAME-2-rbd-$k"
done
}
...
for ((k=1;k<=$RBD_NUMBER_PER_MACHINE;k++));
do
 rbd unmap /dev/rbd/$POOL_NAME/"$MACHINE_NAME-2-rbd-$k"
done
}
...
do
 mkfs.ext4 /dev/rbd/$POOL_NAME/"$MACHINE_NAME-2-rbd-$k"
done
for ((k=6;k<=$RBD_NUMBER_PER_MACHINE;k++));
do
 mkfs.xfs /dev/rbd/$POOL_NAME/"$MACHINE_NAME-2-rbd-$k"
done
}
...
for ((k=1;k<=$RBD_NUMBER_PER_MACHINE;k++));
do
 mount /dev/rbd/$POOL_NAME/"$MACHINE_NAME-2-rbd-$k" "$PERFIX_DIRECTORY""$MACHINE_NAME-$MACHINE_ID/$MACHINE_NAME-$MACHINE_ID-rbd-$k"
done
}
...
for ((k=1;k<=$RBD_NUMBER_PER_MACHINE;k++));
do
 umount /dev/rbd/$POOL_NAME/"$MACHINE_NAME-2-rbd-$k"
done
}
